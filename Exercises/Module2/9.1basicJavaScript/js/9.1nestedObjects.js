const asteroidSection = document.querySelector("#asteroid");
var asteroid = {
  links: {
  ...
  sentry_data:
    "https://api.nasa.gov/neo/rest/v1/neo/sentry/3012393?api_key=6GkozFBru8KqBfF32aHkxtVoL1QpwAsDWU5T5ltz",
};

const name = asteroid.name;
const minDiameter = asteroid.estimated_diameter.miles.estimated_diameter_min;
const maxDiameter = asteroid.estimated_diameter.miles.estimated_diameter_max;
const missDistance = asteroid.close_approach_data[0].miss_distance.miles;

const asteroidInfo = `The asteroid named ${name} is between ${minDiameter} and ${maxDiameter} miles wide, and will miss striking Earth by approximately ${missDistance} miles!`;

asteroidSection.innerHTML = asteroidInfo;

const nasaUrl = asteroid.nasa_jpl_url;
const asteroidLink = `<a href="${nasaUrl}" target="_blank">View more Helen information on NASA JPL website</a>`;
asteroidSection.innerHTML += `<br><br>${asteroidLink}`;
